...
final class ProcessManager: ObservableObject {
    ...
    func launchClone(_ clone: CloneAccount) throws -> Int32 {
        ...
        // Keychain entry "Zalo Safe Storage" đã được fix ACL (-A flag)
        // cho phép TẤT CẢ apps đọc encryption key → clone có thể decrypt session
        // KHÔNG cần --use-mock-keychain hay --password-store=basic nữa
